...
int operateProgram(int id){
    Process p = process[id];
    if (p.isThread || strcmp(p.tag,"TH") == 0){
        int result = operateThread(id);
        return result;
    }
    int index = p.index;
    int instruction = mem[index];
    ...
    int std_out ,
        ...
        while(true){
            ...
            if (value == WaitByThread){
                ...
                addElement(id_running,WBThread);
                id_running = -1;
...
void main(){
    ...
    printf("@%d\n",programs[0].thread_index);
    WBThread = inicializeList();
    ready = inicializeQueue();
    block = inicializeQueue();
    printf("n_instructions: %2d %2d\n",programs[0].n_instructions,programs[0].n_instructions*2);
    printf("n_vars        : %2d\n",programs[0].n_vars);
    printf("total         : %2d\n",programs[0].total);
    printf("t_instructions: %2d\n",programs[0].thread_n_instructions);
    printf("t_index       : %2d\n",programs[0].thread_index);
    printf("t_vars       : %2d\n",programs[0].thread_n_var);
    printf("t_total       : %2d\n",programs[0].thread_total);
    runner();
   //for(int i = 0;i<100;i++){
   //    printf("%d ",mem[i]);
   //}
